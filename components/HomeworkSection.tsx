          ...
          todayHomework.map(hw => (
            ...
            >
              <input
                ...
                className="accent-accent-green w-4 h-4 cursor-pointer shrink-0"
              />
              <span className={`text-xs px-2 py-0.5 rounded border font-medium shrink-0 ${getSubjectColor(hw.subject)}`}>
                {hw.subject}
              </span>
              <span className={`flex-1 min-w-0 text-sm truncate ${hw.done ? 'line-through text-gray-400' : 'text-gray-700'}`}>
                {hw.description}
              </span>
              <button
                onClick={() => onDelete(hw.id)}
                className="text-gray-300 hover:text-red-400 text-xs cursor-pointer shrink-0"
              >
                ✕
              </button>
            </div>
        ...
        <div className="mt-6">
          ...
          <div className="space-y-2">
            {homework.filter(h => h.date < today && !h.done).map(hw => (
              <div key={hw.id} className="flex items-center gap-2 sm:gap-3 p-3 rounded-lg border border-red-200 bg-red-50">
                <input
                  type="checkbox"
                  checked={hw.done}
                  onChange={() => onToggle(hw.id)}
                  className="accent-accent-green w-4 h-4 cursor-pointer shrink-0"
                />
                <span className="text-xs text-gray-400 shrink-0">{hw.date.slice(5)}</span>
                <span className={`text-xs px-2 py-0.5 rounded border font-medium shrink-0 ${getSubjectColor(hw.subject)}`}>
                  {hw.subject}
                </span>
                <span className="flex-1 min-w-0 text-sm text-gray-700 truncate">{hw.description}</span>
                <button
                  onClick={() => onDelete(hw.id)}
                  className="text-gray-300 hover:text-red-400 text-xs cursor-pointer shrink-0"
                >
